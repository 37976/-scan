...
一个本地优先、可安装到手机桌面的文档扫描 PWA。支持拍照或多图导入、直接导入已处理扫描件、四角调整、透视矫正、文档增强、多页管理，以及按自定义顺序选页导出 PDF。
...
Android 版本使用 Capacitor 8，离线扫描流程限定为 `arm64-v8a`，适用于主流 64 位安卓手机。
...
- 多页排序、旋转、删除、重新裁剪
- IndexedDB 本地自动保存
- A4 或原图尺寸 PDF，支持自由选页、按点击顺序排列、页边距、自选保存位置及系统分享
- Service Worker 离线缓存与 PWA 安装

## 隐私

PDF 生成始终在本地完成。Android 版的四角检测、透视校正和精修全部在手机内完成，不上传图片；电脑浏览器开发版则通过同一台电脑上的本地 Node/Python 服务处理，临时文件会在每次处理后删除。
